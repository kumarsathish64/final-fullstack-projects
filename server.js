...
const subjectSchema = new mongoose.Schema({
  ...
  edition: String,
  price: String,
  image: Buffer, // Store the image as binary data (buffer)
  description: String,
});

const Subject = mongoose.model("Subject", subjectSchema);

// POST Route to create a new subject with image upload as buffer
app.post("/api/subjects", upload.single("image"), async (req, res) => {
  try {
    const { course, bookname, author, edition, price, description } = req.body;
...
app.get("/api/subjects", async (req, res) => {
  ...
  try {
    ...
    const subjectsWithImages = subjects.map((subject) => ({
      ...subject.toObject(),
      image: subject.image ? `data:image/jpeg;base64,${subject.image.toString("base64")}` : null,
    }));
...
// GET Route to fetch a single subject by ID
app.get("/api/subjects/:id", async (req, res) => {
  try {
    const subject = await Subject.findById(req.params.id);
    if (!subject) {
    ...
    res.status(200).json(subjectWithImage);
  } catch (error) {
    console.error("Error fetching subject:", error);
    res.status(500).json({ message: "Error fetching subject", error });
  }
...
app.put("/api/subjects/:id", upload.single("image"), async (req, res) => {
  try {
    ...
    const updatedData = {
      ...
      price,
      description,
      ...(image && { image }), // Only update the image if provided
    };
